...
# my_button.pack(pady=100)
    ...
    def make_clickbuttons(self):
        ...
        button_labels = {
            'Cell Reconstruction':[
                ...
                #["Skip Current Cell", 'skip_seg_pr', 0, 17, 1],
                ],
            'Network Exploration': [
        ...
        button_functions = {
            ...
            'choose_save_dir': lambda: Thread(
                target=self.choose_save_directory, 
                args=[], 
                name='choose_save_dir').start(),
            ...
            'load_n_from_seg_id': lambda: Thread(
                target=self.pr_single_neuron_from_seg_id, 
                args=[], 
                name='load_n_from_seg_id').start(),
            
            'browse_from_seg_id': lambda: Thread(
                target=self.browse_from_seg_id, 
                args=[], 
                name='browse_from_seg_id').start(),
            ...
            'choose_browsing_db': lambda: Thread(
                target=self.choose_db, 
                args=['Network Exploration'], 
                name='choose_browsing_db').start(),

            'choose_agglo_db': lambda: Thread(
                target=self.choose_db, 
                args=['Cell Reconstruction'], 
                name='choose_agglo_db').start(),
        ...
        for dtype, col, start_row in (('region', 3, 2), ('type', 2, 2), ('pre_struc_type', 4, 3), ('post_struc_type', 5, 3), ('ei_type', 6, 3)):
            ...
            if dtype == 'ei_type':
                ...
                if dtype in ['region', 'type']:
                    ...
                    query = f"""SELECT {pre_or_post}_class FROM unique_{pre_or_post}_structures_table"""
    ...
    def choose_db(self, mode):
        ...
        self.update_selected_db(selected_file, mode)
...
# paths = browsing_graph_temp.get_all_simple_paths('32326425173', cutoff=8, mode='OUT')
    ...
    def create_cloud_storage_client(self):
        ...
        self.update_mtab('Created new connection to Google Cloud Storage')
    ...
    def seg_pr_batch_start(self, specific_file=None, specific_seg_id=None):
        ...
        required_info = ['cell_structures', 'max_base_seg_add']
    ...
    def assert_segs_in_sync(self, return_segs=False):
        ...
        assert displayed_segs == graph_segs
        assert displayed_segs == listed_segs
    ...
    def add_or_remove_seg(self, action_state):  
        ...
        if base_seg in displayed_segs:
            ...
            if num_base_segs_this_agglo_seg > num_base_segs_not_already_included:
                ...
                print(self.cell_data['removed_base_segs'])
                print(base_ids)
                ...
                print(num_base_segs_this_agglo_seg, num_base_segs_not_already_included, len(base_ids))
