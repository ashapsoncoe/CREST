...
# paths = browsing_graph_temp.get_all_simple_paths('32326425173', cutoff=8, mode='OUT')
    ...
    def get_edges_between_base_segments(self, base_ids):
        ...
        if len(base_ids) > 0:
            ...
            for batch in range(num_batches+1):
                ...
                query = f"""
                            SELECT DISTINCT label_a, label_b
    ...
    def add_new_base_segs_from_new_agglo(self, seg_id):
        ...
        previously_removed_base_segs = set(self.cell_data['removed_base_segs'])
        new_unknown_segs = new_agglo_base_ids - all_current_segs
    
        # This line not used because adding back on only new segments can lead to orphan base segments separated from each other:
        # new_unknown_segs_not_prev_removed = new_unknown_segs - previously_removed_base_segs  
