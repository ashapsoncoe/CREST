...
synapse_biquery_db = 'goog14r0s5c3.synapse_c3_eirepredict_clean_dedup_sep_class_column'
region_and_type_bigquery_db = 'goog14r0seg1.agg20200916c3_regions_types_circ_bounds_20230521'
agglo_seg_info_bigquery_db = 'goog14r0seg1.agg20200916c3_objinfo'
output_db_path = '/home/alexshapsoncoe/drive/CREST_browsing_database_goog14r0s5c3_eirepredict2023.db' #'D:/sqlite_databases/CREST_browsing_database_goog14r0s5c3_spinecorrected_july2022.db' #
cred_path = '/home/alexshapsoncoe/drive/alexshapsoncoe.json' #'C:\\Users\\alexs\\Documents\\h01_work\\alexshapsoncoe.json' # 
temp_store_dir = '/home/alexshapsoncoe/drive/sqlite3_tmp_files' #'D:/sqlite_databases/sqlite3_tmp_files' #


"""
Modified synapse table format created before running script with:

CREATE TABLE IF NOT EXISTS goog14r0s5c3.synapse_c3_eirepredict_clean_dedup_sep_class_column AS (
    SELECT
    CAST(pre_synaptic_site.neuron_id AS INT) AS pre_seg_id, 
    CAST(post_synaptic_partner.neuron_id AS INT) AS post_seg_id, 
    CAST(pre_synaptic_site.id AS INT) AS pre_id, 
    CAST(post_synaptic_partner.id AS INT) AS post_id, 
    LOWER(SUBSTRING(pre_synaptic_site.tags[OFFSET((select offset from unnest(pre_synaptic_site.tags) m with offset where m LIKE '%class_label%'))], 13)) AS pre_struc_type,
    LOWER(SUBSTRING(post_synaptic_partner.tags[OFFSET((select offset from unnest(post_synaptic_partner.tags) m with offset where m LIKE '%class_label%'))], 13)) AS post_struc_type,
    IFNULL(type, 3) AS ei_type,
    CAST(location.x AS INT) AS x,
    CAST(location.y AS INT) AS y,
    CAST(location.z AS INT) AS z
    FROM goog14r0s5c3.synapse_c3_eirepredict_clean_dedup
...
if __name__ == '__main__':
    ...
    query = f"""
            ...
            combo_counts_total AS (
                SELECT * FROM combo_counts_pre WHERE seg_id_1 IS NOT NULL
                UNION DISTINCT
                SELECT * FROM combo_counts_post WHERE seg_id_1 IS NOT NULL
            ),
            ...
            SELECT 
                                ...
                                ei_type
                                FROM {synapse_biquery_db}
                                GROUP BY pre_seg_id, post_seg_id, pre_struc_type, post_struc_type, ei_type
            ...
            combo_counts_total_by_struc_ei AS (
                SELECT * FROM combo_counts_pre_by_struc_ei WHERE seg_id_2 IS NOT NULL
                UNION DISTINCT
                SELECT * FROM combo_counts_post_by_struc_ei WHERE seg_id_2 IS NOT NULL
            ),
            ...
            all_combo_counts AS (
                SELECT
                            ...
                            ON B.seg_id_1 = A.seg_id_2
            ),


            post_site_rts AS (
                    select distinct 
                    A.post_seg_id AS seg_id_3,
                    B.region AS region,
                    B.type AS type,
                    FROM {synapse_biquery_db} A
                    INNER JOIN 
                    {region_and_type_bigquery_db} B
                    ON A.post_seg_id = CAST(B.agglo_id AS INT)
                    ),

            pre_site_rts AS (
                    select distinct
                    A.pre_seg_id AS seg_id_3,
                    B.region AS region,
                    B.type AS type,
                    FROM {synapse_biquery_db} A
                    INNER JOIN 
                    {region_and_type_bigquery_db} B
                    ON A.pre_seg_id = CAST(B.agglo_id AS INT)
                    ),
