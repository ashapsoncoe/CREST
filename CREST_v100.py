...
class UserInterface:
    ...
    def common_browse_start(self):
        ...
        if '_with_skeleton_classes' in self.syn_db_name:
            where_statement =  "WHERE LOWER(pre_synaptic_site.class_label) = 'axon' AND LOWER(post_synaptic_partner.class_label) IN ('dendrite', 'soma')"
        else:
            where_statement = ""
    ...
    def get_synapses_for_set_of_neurons(self, set_of_neurons, pre_or_post, agglo_or_base):
        ...
        info_to_get = [
            ...
            'pre_synaptic_site.neuron_id AS pre_agglo_id',
            'post_synaptic_partner.neuron_id AS post_agglo_id',
            'pre_synaptic_site.base_neuron_id AS pre_base_id',
            'post_synaptic_partner.base_neuron_id AS post_base_id',
        ]

        if '_with_skeleton_classes' in self.syn_db_name:
            info_to_get.append('LOWER(pre_synaptic_site.class_label) AS pre_skel')
            info_to_get.append('LOWER(post_synaptic_partner.class_label) AS post_skel')

        results = self.get_info_from_bigquery(info_to_get, f'CAST({pre_or_post}.{id_key} AS INT64)', set_of_neurons, self.syn_db_name)

        if '_with_skeleton_classes' in self.syn_db_name:
            print('before', len(results))
            results = [x for x in results if str(x['pre_skel']) in ('axon', 'unknown') and str(x['post_skel']) in ('dendrite', 'soma', 'unknown', 'ais', 'axon')]
